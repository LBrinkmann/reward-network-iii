...
from routes.session_utils.session_lifecycle import (
...
@pytest.mark.asyncio
async def test_one_subject_gen_1(
    default_client: httpx.AsyncClient,
    e_config: ExperimentSettings,
    create_empty_experiment,
):
...
async def test_multiple_subjects(
    ...
    for i in range(30, 60):
        task = asyncio.create_task(
            one_subject(default_client, e_config, i, generation=1)
        )
        tasks.append(task)
    [await t for t in tasks]
    ...
    sessions = await Session.find(
        In(Session.subject_id, [s.id for s in subjects]), Session.generation == 1
    ).to_list()
...
async def one_subject(
    ...
    solution = {"moves": [1, 2, 3, 4, 5, 6, 7, 8, 9]}
    written_strategy = {"strategy": "test"}
    ...
    if generation == 0:
        ...
        for i in range(e_config.n_social_learning_blocks):
            ...
            for _ in range(e_config.n_social_learning_networks_per_block):
                ...
                trial_num += 1
                await get_post_trial(
                    default_client, "observation", trial_num, url, solution, headers
                )
                trial_num += 1
                await get_post_trial(
                    default_client, "try_yourself", trial_num, url, solution, headers
    ...
    for i in range(e_config.n_demonstration_trials):
        ...
        await get_post_trial(
            default_client, "demonstration", trial_num, url, solution, headers
        )
        trial_num += 1

    # written strategy trial
    await get_post_trial(
        default_client, "written_strategy", trial_num, url, written_strategy, headers
    )
    trial_num += 1

    await get_post_trial(
        default_client, "post_survey", trial_num, url, post_survey, headers
    )
    trial_num += 1
...
async def get_post_trial(
    ...
    if solution is not None:
        ...
        if trial_type == "social_learning_selection":
            ...
            payload = {"advisor_id": str(trial["advisor_selection"]["advisor_ids"][0])}
...
@pytest.mark.asyncio
async def test_replace_stale_session(
    create_empty_experiment,
    default_client: httpx.AsyncClient,
    e_config: ExperimentSettings,
):
