...
} from "../StaticNetwork/StaticNetwork";
...
export const LinearSolution: FC<LinearSolutionInterface> = (props) => {
  ...
  const setNextNodeColor = useCallback(
    (targetNodeId: number, currentNodeId: number) => {
      const reward = edges.find(
        (edge) =>
          edge.source_num === currentNodeId && edge.target_num === targetNodeId
      )?.reward;
        ...
        <svg width={size.width} height={size.height + gap / 2}>
          <g>
            {moves.map((move, idx) => {
              ...
              const wrongRepeat = correctRepeats && !correctRepeats[idx];
              return (
                <React.Fragment key={"move" + idx}>
                  ...
                  <NetworkNode
                    ...
                    Radius={nodeRadius}
                    onNodeClick={() => {}}
                    strokeColor={wrongRepeat ? "red" : "black"}
                    status={idx > 0 ? "next" : "active"}
                    nextNodeColor={
                      idx > 0 && setNextNodeColor(node.node_num, moves[idx - 1])
                    }
                    isValidMove={false}
...
const LinearSolutionEdge: FC<ILinearSolutionEdge> = (props) => {
  ...
  if (moveIdx < moves.length - 1) {
    const edge = edges.filter(
      (edge) =>
        edge.source_num === moves[moveIdx] &&
        edge.target_num === moves[moveIdx + 1]
    )[0];
