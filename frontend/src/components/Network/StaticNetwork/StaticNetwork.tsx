...
const StaticNetwork: React.FC<StaticNetworkInterface> = (props) => {
  ...
  const setNodeStatus = useCallback(
    (active: boolean, node_num: number) => {
  ...
  const setNextNodeColor = useCallback(
    (node_num: number) => {
      const reward = edges.find(
        (edge) =>
          edge.source_num === currentNodeId && edge.target_num === node_num
      )?.reward;
      ...
      <g filter={blur ? "url(#static-network-blur)" : ""}>
        ...
        <g>
          {edges
            .slice()
            .sort((a, b) => {
              const isACurrent = a.source_num === currentNodeId;
              const isBCurrent = b.source_num === currentNodeId;
              if (isACurrent && !isBCurrent) return 1;
              if (!isACurrent && isBCurrent) return -1;
              return 0;
            })
            .map((edge: StaticNetworkEdgeInterface, idx: number) => {
              return (
                <NetworkEdge
                  reward={edge.reward}
                  edgeWidth={edgeWidth}
