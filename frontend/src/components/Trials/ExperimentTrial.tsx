...
const ExperimentTrial: FC = () => {
  ...
  const submitResults = (result: postTrialType["trialResults"]) => {
    mutation.mutate({
      ...
      trialId: sessionState.currentTrialId,
      trialResults: result,
    });
  };
  ...
  if (status === "loading") {
    ...
    if (!trialData.trial_type) {
      ...
      return (
        <ErrorMessage message={error?.message ? error.message : undefined} />
      );
    }
    ...
    switch (trialData.trial_type) {
      ...
      case TRIAL_TYPE.WRITTEN_STRATEGY:
        return (
          <WrittenStrategyTrial endTrial={submitResults} data={trialData} />
        );
