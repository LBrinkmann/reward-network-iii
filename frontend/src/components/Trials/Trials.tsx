  ...
  return (
    <>
      ...
      <Instruction
        endTrial={props.endTrial}
        instructionType={
          props.data.instruction_type as keyof typeof instructions
        }
      />
    </>
  );
...
export const SelectionTrial: FC<ITrial> = (props) => {
  ...
  const selectAdvisor = (advisorId: string, advisorNumber: number) => {
    sessionDispatcher({
      ...
      payload: {
        selectedAdvisor: { advisorId: advisorId, advisorNumber: advisorNumber },
      },
    });
...
export const ObservationTrial: FC<ITrial> = (props) => {
  const [isTimeoutAfterLastMoveDone, setIsTimeoutAfterLastMoveDone] =
    useState(false);
  ...
  if (
    ...
    return (
      <>
        <Header title={props.data.trial_title} />
        <Observation
          solution={props.data.advisor.solution.moves}
...
export const TryYourselfTrial: FC<ITrial> = (props) => {
  ...
  const calculateScore = useCallback(
    (moves: number[], edges: StaticNetworkEdgeInterface[]) => {
      let score = 0;
      for (let i = 0; i < moves.length - 1; i++) {
        const edge = edges.find(
          (e) => e.source_num === moves[i] && e.target_num === moves[i + 1]
        );
        if (edge) {
          score += edge.reward;
        }
  ...
  if (
    ...
    return (
      <WaitForNextTrialScreen
        newNetwork={sessionState.lastTrialForCurrentExample}
      />
    );
  else
    return (
      <>
        ...
        <TryYourself
          solution={props.data.advisor.solution.moves}
          teacherTotalScore={calculateScore(
            props.data.advisor.solution.moves,
            props.data.network.edges
          )}
          teacherId={sessionState.selectedAdvisor.advisorNumber}
    ...
    return (
      <>
        ...
        <Typography variant="h4" align="center">
          Collect points based on arrow colors
        </Typography>
        <NetworkTrial
          playerTotalPoints={sessionState.totalPoints}
...
export const DemonstrationTrial: FC<ITrial> = (props) => {
  ...
  if (!networkState.network) return <WaitForNextTrialScreen />;
  else if (networkState.isNetworkFinished && isTimeoutAfterLastMoveDone)
...
export const WrittenStrategyTrial: FC<ITrial> = (props) => {
  ...
  return (
    <>
      <Header title={"Your Strategy"} />
      <WrittenStrategy
        endTrial={props.endTrial}
        type={sessionState.currentTrialId < 10 ? "start" : "end"}
      />
    </>
  );
};
