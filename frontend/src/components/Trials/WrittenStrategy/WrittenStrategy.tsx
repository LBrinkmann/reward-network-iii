...
const WrittenStrategy: React.FC<WrittenStrategyInterface> = ({
  endTrial,
  type = "end",
}) => {
  ...
  const onChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.value.length <= 200)
      setWrittenStrategy(event.target.value);
  };
      ...
      <Grid sx={{ flexGrow: 1 }} direction="column" container spacing={4}>
        <Grid item style={{ textAlign: "center" }}>
          <Typography variant="h5" component="div">
            {type === "start"
              ? "Please think about a strategy for the network task that maximizes points. What did you observe as successful?"
              : "Please think about a strategy for the network task that maximizes points. What did you observe as successful?"}
          </Typography>
        </Grid>
        <Grid item style={{ margin: 8 }}>
          <TextField
            ...
            helperText={
              type == "start"
                ? "Please enter between 20 and 200 characters"
                : "Please enter between 20 and 200 characters"
            }
