...
with st.expander("Compare strategies 🤖"):
    if "networks" in st.session_state:
        ...
        strategy_data_final = strategy_data[
            strategy_data["step"] == st.session_state.gen_env.n_steps
        ]
...
with st.expander("Show solution dataframes 📊"):
    if "networks" in st.session_state:
        ...
        with col1:
            avg_val1 = (
                m_df[m_df["step"] == st.session_state.gen_env.n_steps]["total_reward"]
                .mean()
                .round(0)
            )
        ...
        with col2:
            avg_val2 = (
                l_df[l_df["step"] == st.session_state.gen_env.n_steps]["total_reward"]
                .mean()
                .round(0)
            )
